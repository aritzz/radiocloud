...
} else if (($_GET['t'] == "gehitu_local"))
{
	...
	if ($_POST) {
		...
		if (!empty($izena) && !empty($erabiltzailea) && !empty($pasahitza) && !empty($maila) && ($current_user->login_level >= $maila))
		if (!$db->get_var("SELECT username FROM users WHERE username='$erabiltzailea'")) {
			$db->query("INSERT INTO users VALUES(NULL, '$erabiltzailea', MD5('$pasahitza'), '$izena', '$desc', 0, 1, 'internal', 'img/default.png', '$maila')");
	...
	$pasahitza = $db->escape($_POST['pass']);
	...
	$arrosac = $db->escape($_POST['arrosa_category']);
	...
	if (!$db->get_var("SELECT username FROM users WHERE username='$erabiltzailea'")){
		...
		/*
			Prozesatu irudia
		*/
	    $argazkia = "img/default.png";
		if ($_FILES["argazkia"]["size"] > 500000) {
			$argazkia = "img/default.png";
		} else if ($_FILES["argazkia"]["size"] == 0) {
			$argazkia = "img/default.png";
		} 
		else {
			...
			if (move_uploaded_file($_FILES["argazkia"]["tmp_name"], $argafile))
				$argazkia = "argazkiak/$erabiltzailea.png";
		...
		if($db->query('COMMIT') !== false) {
		    ...
		    foreach ($defaults as $default) 
                $db->query("INSERT INTO access VALUES('$insertuser_id', ".$default->access_module.")"); // profile
            
            $defaults = $db->get_results("SELECT * FROM access_menu WHERE userid = -1");
		    foreach ($defaults as $default) 
                $db->query("INSERT INTO access_menu VALUES('$insertuser_id', ".$default->menuid.")"); // profile
            
            
			/*$db->query("INSERT INTO access VALUES('$insertuser_id', 5)"); // profile
			$db->query("INSERT INTO access VALUES('$insertuser_id', 2)"); // config

			$db->query("INSERT INTO access_menu VALUES('$insertuser_id', 6)"); // config
			$db->query("INSERT INTO access_menu VALUES('$insertuser_id', 7)"); // config
			$db->query("INSERT INTO access_menu VALUES('$insertuser_id', 9)"); // config
